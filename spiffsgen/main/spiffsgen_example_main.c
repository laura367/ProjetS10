...
{
    ESP_LOGI(TAG, "Reading audio_file");

    // Open for reading hello.txt
    FILE* f = fopen("/spiffs/hello.wav", "r");
    if (f == NULL) {
        ESP_LOGE(TAG, "Failed to open hello.wav");
        return;
    }
    ...
    ESP_LOGI(TAG, "Read from hello.wav: %s", buf);
}

static void compute_alice_txt_md5(void)
{
    ESP_LOGI(TAG, "Computing test_txt MD5 hash");

    // The file alice.txt lives under a subdirectory, though SPIFFS itself is flat
    FILE* f = fopen("/spiffs/sub/alice.txt", "rb");
    if (f == NULL) {
